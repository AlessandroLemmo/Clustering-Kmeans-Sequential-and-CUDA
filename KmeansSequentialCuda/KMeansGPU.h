...
inline __global__ void FindCluster(Point* d_points, int* d_clusters, Point* d_result, Point* d_new_result, int* d_clusterCounts, int* d_delta, int* d_k, int* d_size, int threads)
{
	...
	for (int cluster = 0; cluster < k; cluster++)
	{
		...
		//numero di punti appartenenti ad ogni cluster inizializzati a 0
		d_clusterCounts[cluster * threads + i] = 0;						
	}

	//assegnazione del punto i al cluster j
	//ciclo sul numero di punti di un thread
	for (int pointNr = 0; pointNr < POINTS_FOR_THREAD; pointNr++)		
	{
		...
		for (int j = 0; j < k; j++)										
		{
			...
			{
				minDist = dist;
				//trovo per il punto index-esimo il miglior cluster
				bestCluster = j;										
			}
		}
		...
		if (bestCluster != d_clusters[index])
		{
			...
			//d_delta indica il numero di punti cambiati di cluster ad ogni iterazione
			d_delta[index] = 1;											
		}
		//numero di punti per cluster
		d_clusterCounts[bestCluster * threads + i]++;					
...
{
	//indice del cluster di appartenenza di ogni punto
	int* h_clusters;
	//centroidi risultati
	Point* h_result;										
	...
	//   size/POINTS_PER_THREAD =  numero di threads da usare
	//   /nThreads = numero di threads per blocco
	int nBlocks = size / POINTS_FOR_THREAD / nThreads;		
															
	nBlocks += (size % nThreads == 0) ? 0 : 1;

	//blocchi per clusters
	//numero di blocchi per gestire tutti i centroidi 
